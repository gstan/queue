...
class RedisClient
{
    ...
    {
        $num = count($this->pool);
        $log = "redis connection:" . $num;
        \Lib\Log::instance()->write($log);
        if ($num > 0)
        {
            /**
...
class RedisConnection
{
    ...
    {
        \Lib\Log::instance()->write("on Connect:". $this->wait_send);
        if ($this->wait_send)
        {
    ...
    {
        \Lib\Log::instance()->write("on Receive" . $data);
        $success = true;
        if ($this->redis->debug)
        {
    ...
    {
        \Lib\Log::instance()->write("on Close:" . $this->wait_send);
        if ($this->wait_send)
        {
            $this->redis->freeConnection($cli->sock, $this);
